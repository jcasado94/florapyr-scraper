# given a list of species IDs (prefered) or names in input.txt, returns information about the observations for each of them.
...
setSpeciesURL = "http://www.atlasflorapyrenaea.eu/src/commun/liste-taxon-process.php"
...
# False: input as species FloraPyr ID (prefered)
...
try:
  s.get(initalRequestURL, timeout = 10)
...
for sp in sps:
  try:
    r = s.post(setSpeciesURL, data = {'cd_ref_flpyr': sp, 'submit': '1'}, timeout = 10)
  except requests.exceptions.Timeout:
...
# get observations
try:
  r = s.get(getObservationsURL, timeout = 10)
except requests.exceptions.Timeout:
